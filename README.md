...
* **record**
  ...
  Records (appends) data from STDIN to a file. Expects a single required argument, the `file_path`. Optionally accepts `--rotate-interval` (hourly, daily, weekly, or monthly) and `--rotate-count` (number of rotated files to keep, defaults to 7) to configure automatic log rotation using logrotate and cronjob.
...
services:
    ...
    record_1:
        ...
        command: ["from_bus 1 | record /recordings/bus_id_1.log --rotate-interval daily --rotate-count 30"]
